...
#[vitrail(schema = crate::d1_test_schema::Schema, model = scalar_record)]
struct ScalarRecord {
    id: i64,
    min_value: i64,
    max_value: i64,
...
#[derive(Clone, Debug, QueryResult)]
#[vitrail(
    schema = crate::d1_test_schema::Schema,
    model = scalar_record,
...
#[derive(Clone, Debug, QueryResult)]
#[vitrail(schema = crate::d1_test_schema::Schema, model = author)]
...
async fn run_crud_probe(env: &Env) -> WorkerResult<Response> {
    ...
    let inserted = client
        .insert(d1_test_schema::insert::<InsertedScalarRecord>(
            NewScalarRecord {
                min_value: i64::MIN,
                max_value: i64::MAX,
                active: true,
    ...
    let queried = client
        .find_first(d1_test_schema::query_with_variables::<RecordByMax>(
    ...
    let updated = client
        .update_many(d1_test_schema::update_many_with_variables::<
    ...
    let deleted = client
        .delete_many(d1_test_schema::delete_many_with_variables::<
    ...
    let remaining = client
        .find_optional(d1_test_schema::query::<ScalarRecord>())
        .await
        .map_err(worker_error)?;
...
INSERT INTO "wide_child" ("id", "parent_id", {}) VALUES ({wide_child_id}, {wide_parent_id}, {});"#,
    ...
    let post_with_author = client
        .find_first(d1_test_schema::query_with_variables::<PostWithAuthor>(
            PostIdVariables {
    ...
    for (label, note, min_value, max_value) in [
        ...
        client
            .insert(d1_test_schema::insert::<InsertedScalarRecord>(record))
            .await
            .map_err(worker_error)?;
    }
    ...
    let accepted_bindings = client
        .find_many(d1_test_schema::query_with_variables::<RecordIdOnly>(
            RecordIdsVariables {
...
async fn run_direct_decode_error_probe(env: &Env) -> WorkerResult<Response> {
    ...
    let decode_error = match client
        .find_many(d1_test_schema::query_with_variables::<RecordByLabel>(
...
async fn run_session_probe(env: &Env) -> WorkerResult<Response> {
    ...
    let unconstrained_second = first_unconstrained
        .find_optional(d1_test_schema::query_with_variables::<RecordByMax>(
            RecordByMaxVariables {
                max_value: i64::MAX,
            },
        ))
...
async fn run_atomic_batch_probe(env: &Env) -> WorkerResult<Response> {
    ...
    let many_handle = batch
        .find_many(d1_test_schema::query_with_variables::<RecordByMax>(
            RecordByMaxVariables {
                max_value: i64::MAX,
            },
        ))
    ...
    let rejected_binding_error = match limit_batch.find_many(
        d1_test_schema::query_with_variables::<RecordIdOnly>(RecordIdsVariables {
            record_ids: (0_i64..101_i64).collect(),
        }),
    ) {
        Ok(_) => {
...
async fn run_atomic_batch_rollback_probe(env: &Env) -> WorkerResult<Response> {
    ...
    let rows_after_binding_failure = client
        .find_many(d1_test_schema::query::<ScalarRecord>())
        .await
        .map_err(worker_error)?;
    ensure(
