...
use crate::{
    AtomicBatch, BatchOutput, D1Executor, DeleteSpec, Error, InsertSpec, QuerySpec, UpdateSpec,
};
...
impl VitrailSession {
    ...
    /// Queues and executes a typed atomic batch through this logical D1
    /// session.
    ///
    /// D1 executes the queued operations atomically and in order through one
    /// session `batch()` call. The callback only builds that batch and does
