...
/// Backend-specific validation hooks used by the shared schema infrastructure.
#[doc(hidden)]
pub trait DialectPolicy: Clone + Copy + fmt::Debug + Default + Eq + Send + Sync + 'static {
...
/// Core-owned dialect marker that delegates validation to a backend policy.
#[doc(hidden)]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct DialectMarker<P: DialectPolicy>(PhantomData<fn() -> P>);

impl<P: DialectPolicy> private::Sealed for DialectMarker<P> {}

/// Sealed dialect identity carried by shared schema values.
#[doc(hidden)]
pub trait Dialect:
    private::Sealed + Clone + Copy + fmt::Debug + Default + Eq + Send + Sync + 'static
{
    fn validate_scalar_type(scalar: ScalarType) -> Result<(), String>;
...
impl<P: DialectPolicy> Dialect for DialectMarker<P> {
    fn validate_scalar_type(scalar: ScalarType) -> Result<(), String> {
        P::validate_scalar_type(scalar)
    }
    ...
    fn validate_native_attribute(
        ...
        P::validate_native_attribute(attribute, field_type)
    }
    ...
    fn normalize_external_table_name(table: &str) -> Result<String, String> {
        P::normalize_external_table_name(table)
    }
}

/// Native field attributes understood by the shared validation machinery.
#[doc(hidden)]
...
/// Dialect-marked schema definition.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Schema<D: Dialect> {
    models: Vec<Model<D>>,
    external_tables: Vec<String>,
}

impl<D: Dialect> Schema<D> {
    pub fn builder() -> SchemaBuilder<D> {
        SchemaBuilder::new()
    }

    pub fn models(&self) -> &[Model<D>] {
        &self.models
    }
    ...
    pub fn model(&self, name: &str) -> Option<&Model<D>> {
    ...
    pub(crate) fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        for table in &self.external_tables {
            let normalized = match D::normalize_external_table_name(table) {
    ...
    #[doc(hidden)]
    pub fn resolve_model(&self, requested: &str) -> Resolution<'_, D> {
...
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct SchemaBuilder<D: Dialect> {
    models: Vec<Model<D>>,
    external_tables: Vec<String>,
}

impl<D: Dialect> SchemaBuilder<D> {
    pub fn new() -> Self {
        Self::default()
    }

    pub fn model(mut self, model: Model<D>) -> Self {
        self.models.push(model);
        self
    }

    pub fn models(mut self, models: Vec<Model<D>>) -> Self {
        self.models = models;
        self
    }
    ...
    pub fn build(self) -> Result<Schema<D>, ValidationErrors> {
        let schema = Schema {
            models: self.models,
...
pub struct Model<D: Dialect> {
    name: String,
    fields: Vec<Field<D>>,
    attributes: Vec<ModelAttribute<D>>,
}

impl<D: Dialect> Model<D> {
    pub fn builder(name: impl Into<String>) -> ModelBuilder<D> {
    ...
    pub fn fields(&self) -> &[Field<D>] {
        &self.fields
    }

    pub fn attributes(&self) -> &[ModelAttribute<D>] {
        &self.attributes
    }
    ...
    fn primary_key_attribute(&self) -> Option<&ModelPrimaryKeyAttribute<D>> {
        self.attributes
            .iter()
    ...
    fn validate_types(&self, schema: &Schema<D>, errors: &mut Vec<ValidationError>) {
        for field in &self.fields {
            field.validate_type(schema, &self.name, errors);
        }
    }

    fn validate_relations(&self, schema: &Schema<D>, errors: &mut Vec<ValidationError>) {
        for field in &self.fields {
    ...
    pub fn field_named(&self, name: &str) -> Option<&Field<D>> {
        self.fields.iter().find(|field| field.name == name)
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ModelBuilder<D: Dialect> {
    name: String,
    fields: Vec<Field<D>>,
    attributes: Vec<ModelAttribute<D>>,
}

impl<D: Dialect> ModelBuilder<D> {
    ...
    pub fn field(mut self, field: Field<D>) -> Self {
        self.fields.push(field);
        self
    }

    pub fn fields(mut self, fields: Vec<Field<D>>) -> Self {
        self.fields = fields;
        self
    }

    pub fn attribute(mut self, attribute: ModelAttribute<D>) -> Self {
        self.attributes.push(attribute);
        self
    }

    pub fn attributes(mut self, attributes: Vec<ModelAttribute<D>>) -> Self {
        self.attributes = attributes;
        self
    }

    pub fn build(self) -> Result<Model<D>, ValidationErrors> {
        let model = Model {
            name: self.name,
            fields: self.fields,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Field<D: Dialect> {
    name: String,
    ty: FieldType,
    attributes: Vec<Attribute<D>>,
}

impl<D: Dialect> Field<D> {
    pub fn builder(name: impl Into<String>, ty: FieldType) -> FieldBuilder<D> {
    ...
    pub fn attributes(&self) -> &[Attribute<D>] {
        &self.attributes
    }
    ...
    pub fn relation(&self) -> Option<&RelationAttribute<D>> {
        self.attributes
            .iter()
            .find_map(|attribute| match attribute {
    ...
    pub fn rust_type(&self) -> Option<&RustTypeAttribute<D>> {
        self.attributes
            .iter()
            .find_map(|attribute| match attribute {
    ...
    fn validate_attributes(&self, model_name: &str, errors: &mut Vec<ValidationError>) {
        if let FieldType::Scalar(scalar) = &self.ty
            && let Err(message) = D::validate_scalar_type(scalar.scalar())
        {
            errors.push(ValidationError::new(
                ValidationLocation::FieldType {
                    model: model_name.to_owned(),
                    field: self.name.clone(),
                    ty: scalar.scalar().as_str().to_owned(),
                },
                message,
            ));
        }
        ...
        for attribute in &self.attributes {
            match attribute {
                ...
                Attribute::DbUuid => {
                    ...
                    if let Err(message) =
                        D::validate_native_attribute(NativeAttribute::DbUuid, &self.ty)
                    {
                        errors.push(ValidationError::new(
                            ValidationLocation::Attribute {
                                model: model_name.to_owned(),
    ...
    fn validate_default(
        &self,
        model_name: &str,
        default: &DefaultAttribute<D>,
        errors: &mut Vec<ValidationError>,
    ) {
        if let Err(message) = D::validate_default(&self.ty, default.function()) {
            errors.push(ValidationError::new(
                ValidationLocation::Attribute {
                    model: model_name.to_owned(),
                    field: self.name.clone(),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct FieldBuilder<D: Dialect> {
    name: String,
    ty: FieldType,
    attributes: Vec<Attribute<D>>,
}

impl<D: Dialect> FieldBuilder<D> {
    ...
    pub fn attribute(mut self, attribute: Attribute<D>) -> Self {
        self.attributes.push(attribute);
        self
    }

    pub fn attributes(mut self, attributes: Vec<Attribute<D>>) -> Self {
        self.attributes = attributes;
        self
    }

    pub fn build(self) -> Result<Field<D>, ValidationErrors> {
        self.build_for_model("<field>")
    }

    pub fn build_for_model(self, model_name: &str) -> Result<Field<D>, ValidationErrors> {
        let field = Field {
            name: self.name,
            ty: self.ty,
            attributes: self.attributes,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Attribute<D: Dialect> {
    Id,
    Unique,
    Index,
    Default(DefaultAttribute<D>),
    Relation(RelationAttribute<D>),
    DbUuid,
    RustType(RustTypeAttribute<D>),
}

#[derive(Clone, Eq, PartialEq)]
pub struct RustTypeAttribute<D: Dialect> {
    path: String,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for RustTypeAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RustTypeAttribute")
            .field("path", &self.path)
            .finish()
    }
}

impl<D: Dialect> RustTypeAttribute<D> {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ModelAttribute<D: Dialect> {
    Id(ModelPrimaryKeyAttribute<D>),
    Unique(ModelUniqueAttribute<D>),
    Index(ModelIndexAttribute<D>),
}

#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelPrimaryKeyAttribute<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelPrimaryKeyAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelPrimaryKeyAttribute")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelPrimaryKeyAttribute<D> {
    pub fn builder() -> ModelPrimaryKeyAttributeBuilder<D> {
    ...
    fn validate(&self, model: &Model<D>, errors: &mut Vec<ValidationError>) {
        if self.fields.is_empty() {
            errors.push(ValidationError::new(
                ValidationLocation::ModelAttribute {
                    model: model.name.clone(),
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelPrimaryKeyAttributeBuilder<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelPrimaryKeyAttributeBuilder<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelPrimaryKeyAttributeBuilder")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelPrimaryKeyAttributeBuilder<D> {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(self) -> Result<ModelPrimaryKeyAttribute<D>, ValidationErrors> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelUniqueAttribute<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelUniqueAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelUniqueAttribute")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelUniqueAttribute<D> {
    pub fn builder() -> ModelUniqueAttributeBuilder<D> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelUniqueAttributeBuilder<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelUniqueAttributeBuilder<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelUniqueAttributeBuilder")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelUniqueAttributeBuilder<D> {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(self) -> Result<ModelUniqueAttribute<D>, ValidationErrors> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelIndexAttribute<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelIndexAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelIndexAttribute")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelIndexAttribute<D> {
    pub fn builder() -> ModelIndexAttributeBuilder<D> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct ModelIndexAttributeBuilder<D: Dialect> {
    fields: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for ModelIndexAttributeBuilder<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ModelIndexAttributeBuilder")
            .field("fields", &self.fields)
            .finish()
    }
}

impl<D: Dialect> ModelIndexAttributeBuilder<D> {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(self) -> Result<ModelIndexAttribute<D>, ValidationErrors> {
...
#[derive(Clone, Eq, PartialEq)]
pub struct DefaultAttribute<D: Dialect> {
    function: DefaultFunction,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for DefaultAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DefaultAttribute")
            .field("function", &self.function)
            .finish()
    }
}

impl<D: Dialect> DefaultAttribute<D> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct RelationAttribute<D: Dialect> {
    fields: Vec<String>,
    references: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for RelationAttribute<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RelationAttribute")
            .field("fields", &self.fields)
...
impl<D: Dialect> RelationAttribute<D> {
    pub fn builder() -> RelationAttributeBuilder<D> {
...
#[derive(Clone, Default, Eq, PartialEq)]
pub struct RelationAttributeBuilder<D: Dialect> {
    fields: Vec<String>,
    references: Vec<String>,
    dialect: PhantomData<fn() -> D>,
}

impl<D: Dialect> fmt::Debug for RelationAttributeBuilder<D> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RelationAttributeBuilder")
            .field("fields", &self.fields)
            .field("references", &self.references)
            .finish()
    }
...
impl<D: Dialect> RelationAttributeBuilder<D> {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn build(self) -> Result<RelationAttribute<D>, ValidationErrors> {
    ...
    pub fn build_for_field(
        ...
        model_name: &str,
        field_name: &str,
    ) -> Result<RelationAttribute<D>, ValidationErrors> {
...
pub enum Resolution<'a, D: Dialect> {
    Found(&'a Model<D>),
    NotFound,
    Ambiguous(Vec<&'a Model<D>>),
}
