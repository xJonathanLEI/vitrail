...
use vitrail_sqlite_core::{SchemaAccess, SqliteMigrator};
...
#[command(about = "Vitrail CLI for schema-aware SQLite migrations")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
struct MigrationConnectionArgs {
    #[arg(long, env = "VITRAIL_DATABASE_URL", help = "SQLite database URL")]
    database_url: String,
    #[arg(
        long,
...
impl MigrationConnectionArgs {
    fn migrator(&self) -> SqliteMigrator {
        SqliteMigrator::new(self.database_url.clone(), self.migrations_path.clone())
    }
