...
use vitrail_sqlite::{
    QueryResult, QueryVariables, SqliteSchema, StringValueType, UpdateData, UpdateMany,
...
pub(crate) use self::custom_types_schema as sqlite_custom_types_schema;
...
async fn setup_database(database_url: &str) -> i64 {
    apply_schema(
        database_url,
        &SqliteSchema::from_schema_access::<crate::custom_types_schema::Schema>(),
    )
    .await;
