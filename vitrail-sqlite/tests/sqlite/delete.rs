...
use vitrail_sqlite::{
    DeleteMany, DeleteManyModel, QueryFilter, QueryFilterValue, QueryVariableValue, QueryVariables,
    SqliteSchema, VitrailClient, delete, schema,
};
...
pub(crate) use self::delete_schema as sqlite_delete_schema;

#[derive(vitrail_sqlite::QueryVariables)]
...
async fn setup_database(database_url: &str) {
    apply_schema(
        database_url,
        &SqliteSchema::from_schema_access::<crate::delete_schema::Schema>(),
    )
    .await;
...
async fn delete_many_deletes_multiple_rows_and_returns_count() {
    ...
    let remaining_comments: i64 = sqlx::query_scalar(r#"SELECT COUNT(*) FROM "comment""#)
        .fetch_one(&pool)
        .await
...
fn delete_many_generates_expected_sql_for_nested_relation_filter() {
    ...
    assert_eq!(
        ...
        [
            ...
            r#"WHERE EXISTS (SELECT 1 FROM "post" AS "t1" WHERE "t1"."id" = "t0"."post_id" AND EXISTS (SELECT 1 FROM "user" AS "t2" WHERE "t2"."id" = "t1"."author_id" AND "t2"."age" = ?1))"#,
        ]
        .join(" ")
    );
}
