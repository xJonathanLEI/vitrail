...
pub(crate) use self::bigint_schema as sqlite_bigint_schema;
...
fn bigint_columns_generate_bigint_migration_sql() {
    let sql = SqliteSchema::from_schema_access::<crate::bigint_schema::Schema>()
...
async fn bigint_columns_work_across_relations_query_and_update() {
    ...
    apply_schema(
        &database_url,
        &SqliteSchema::from_schema_access::<crate::bigint_schema::Schema>(),
    )
    .await;
    ...
    .fetch_one(&pool)
    .await
    .expect("should read updated bigint invoice directly from SQLite");
