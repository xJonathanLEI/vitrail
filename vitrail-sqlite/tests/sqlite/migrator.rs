...
use vitrail_sqlite::{MigrationSource, SqliteMigrator, SqliteSchema, embed_migrations, schema};
...
async fn apply_all_accepts_embedded_migrations_without_filesystem_source() {
    ...
    let migrator = SqliteMigrator::embedded(
        database.url(),
        [(
...
"#,
    ...
    let mut connection = SqliteConnection::connect(database.url())
        .await
        .expect("should connect to migrated database");

    let embedded_table_exists = sqlx::query_scalar::<_, i64>(
        r#"
SELECT EXISTS (
    SELECT 1
    FROM sqlite_master
    WHERE type = 'table'
      AND name = 'embedded_user'
)
"#,
    )
...
async fn generate_migration_writes_directory_for_pending_change() {
    ...
    let migrator = SqliteMigrator::new(database.url(), &migrations_path);
    ...
    let expected_sql = SqliteSchema::from_schema_access::<migrator_base_schema::Schema>()
    ...
    let target_database = SqliteSchema::introspect(database.url())
        .await
        .expect("should introspect target database after migration generation");
    assert!(
        target_database.tables().is_empty(),
        "migration generation must not inspect or mutate the target database"
    );
...
async fn apply_all_replays_generated_migrations_and_tracks_history() {
    ...
    let current = SqliteSchema::introspect(database.url())
        .await
        .expect("should introspect migrated database");
    let target = SqliteSchema::from_schema_access::<migrator_expanded_schema::Schema>();
...
fn temporary_migrations_path(label: &str) -> PathBuf {
    std::env::temp_dir().join(format!(
        "vitrail_sqlite_migrator_{label}_{}",
        unique_suffix()
    ))
}
