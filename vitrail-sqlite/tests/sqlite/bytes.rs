...
use vitrail_sqlite::{
    QueryResult, QueryVariables, SqliteSchema, UpdateData, UpdateMany, VitrailClient, insert,
    schema,
};
...
pub(crate) use self::bytes_schema as sqlite_bytes_schema;
...
#[test]
fn bytes_columns_generate_blob_migration_sql() {
    let sql = SqliteSchema::from_schema_access::<crate::bytes_schema::Schema>()
        .migrate_from(&SqliteSchema::empty())
        .to_sql();

    assert!(sql.contains(r#""data" BLOB NOT NULL"#));
    assert!(sql.contains(r#""checksum" BLOB"#));
    assert!(sql.contains(r#""payload" BLOB NOT NULL"#));
}
...
async fn bytes_columns_work_across_insert_query_update_and_include() {
    ...
    apply_schema(
        &database_url,
        &SqliteSchema::from_schema_access::<crate::bytes_schema::Schema>(),
    )
    .await;
    ...
    .fetch_one(&pool)
    .await
    .expect("should read stored bytes directly from SQLite");
