...
use vitrail_sqlite::{
    DeleteMany, InsertInput, InsertResult, QueryResult, QueryVariables, SqliteSchema,
    TransactionMode, TransactionOptions, UpdateData, UpdateMany, VitrailClient, VitrailTransaction,
    schema,
};
...
async fn setup_database(database_url: &str) {
    apply_schema(
        database_url,
        &SqliteSchema::from_schema_access::<crate::transaction::transaction_schema::Schema>(),
    )
    .await;
}
...
#[tokio::test]
async fn transaction_begin_with_immediate_mode_succeeds() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    ...
    let txn = client
        .begin_with_options(TransactionOptions::with_mode(TransactionMode::Immediate))
        .await
        .expect("should begin transaction with immediate mode");
