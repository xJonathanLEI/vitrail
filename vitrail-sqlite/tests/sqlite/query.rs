...
#[tokio::test]
async fn simple_query_on_sqlite() {
...
#[tokio::test]
async fn model_first_named_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    let author_id = setup_database(&database_url).await;
...
#[tokio::test]
async fn nested_model_first_where_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    let author_id = setup_database(&database_url).await;
...
#[tokio::test]
async fn model_first_to_one_include_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    let author_id = setup_database(&database_url).await;
...
#[tokio::test]
async fn model_first_recursive_nested_include_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    let author_id = setup_database(&database_url).await;
...
async fn setup_compound_database(database_url: &str) -> i64 {
    apply_schema(
        database_url,
        &SqliteSchema::from_schema_access::<crate::query::compound_query_schema::Schema>(),
    )
    .await;
...
#[tokio::test]
async fn compound_to_one_include_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
...
#[tokio::test]
async fn compound_to_many_include_query_on_sqlite() {
    let database = TestDatabase::new();
    let database_url = database.url().to_owned();
    let post_id = setup_compound_database(&database_url).await;
